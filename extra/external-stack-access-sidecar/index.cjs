...
function validateCompose(compose) {
    ...
    fs.realpathSync(compose.workingDir);
    if (!Array.isArray(compose.configFiles) || compose.configFiles.length === 0 || compose.configFiles.length > 16) throw new Error("Dockge-Enhanced Compose config is missing or too large");
    for (const file of compose.configFiles) {
        if (!path.isAbsolute(file) || path.resolve(file) !== file) throw new Error("Dockge-Enhanced Compose config path is invalid");
        requireRegularFile(file, "Compose config");
    }
    if (!Array.isArray(compose.envFiles)) compose.envFiles = [];
    if (compose.envFiles.length > 8) throw new Error("Too many Dockge-Enhanced env files");
    for (const file of compose.envFiles) {
        if (!path.isAbsolute(file) || path.resolve(file) !== file) throw new Error("Dockge-Enhanced env-file path is invalid");
        requireRegularFile(file, "Compose env file");
    }
}
...
function readAndClaimPlan(planPath = process.env.EXTERNAL_STACK_ACCESS_PLAN) {
    ...
    const requestedPaths = Array.isArray(plan.requestedPaths) && plan.requestedPaths.length > 0
        ? plan.requestedPaths
        : [ { path: plan.requestedPath, addBind: plan.addBind === true } ];
    if (requestedPaths.length > 64) throw new Error("Too many external paths requested");
    for (const entry of requestedPaths) {
        if (!entry || typeof entry !== "object" || typeof entry.addBind !== "boolean" || /[\0\r\n,]/.test(entry.path) || !path.isAbsolute(entry.path) || path.resolve(entry.path) !== entry.path || entry.path === "/") {
            throw new Error("Requested external-path operation is invalid");
        }
    }
...
function mergeAllowedPaths(current, requestedPaths) {
...
function patchDocument(raw, plan) {
    ...
    if (bindsToAdd.length > 0) {
        ...
        for (const requested of bindsToAdd) {
            const covered = volumes.items.some((item) => {
                const mount = bindMount(item?.toJSON());
                return mount?.source && mount.source === mount.target && inside(mount.target, requested.path);
            });
            if (!covered) volumes.add(doc.createNode({ type: "bind", source: requested.path, target: requested.path }));
        }
    }

    const allowedPaths = requestedPaths.map((entry) => entry.path);
    ...
    if (YAML.isMap(environment)) {
        environment.set("DOCKGE_EXTERNAL_STACKS_ALLOWED_PATHS", mergeAllowedPaths(environment.get("DOCKGE_EXTERNAL_STACKS_ALLOWED_PATHS"), allowedPaths));
        ...
        const updated = `${prefix}${mergeAllowedPaths(current, allowedPaths)}`;
...
function composeArgs(compose) {
    ...
    for (const file of compose.envFiles || []) args.push("--env-file", file);
...
function accessApplied(plan, inspected) {
    ...
    for (const requested of plan.requestedPaths || []) {
        if (!allowed.includes(requested.path)) return false;
        if (!requested.addBind) continue;
        const covered = mounts.some((mount) => mount?.Type === "bind" && mount.Source && mount.Destination
            && path.resolve(mount.Source) === path.resolve(mount.Destination)
            && inside(path.resolve(mount.Destination), requested.path));
...
function applyComposeAccess(plan, deps = {}) {
    ...
    for (const file of plan.compose.configFiles) {
        ...
        if (patched === null || patchedFile) continue;
        patchedFile = file;
