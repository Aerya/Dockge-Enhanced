...
Dockge-Enhanced 现在提供可从任意页面打开的全局搜索面板。它会并行搜索本地实例和所有在线关联实例，可查找 **Stack**、安全的 **Compose/override** 匹配、**`.env` 变量名**以及**备份历史元数据**，同时提供主要 Enhanced 配置页面的快捷入口。结果可按类别或实例筛选，并可直接打开对应 Stack 或页面。搜索从两个字符开始，带有 debounce、每实例结果限制，并短暂缓存 Stack 文档以避免输入过程中反复扫描相同 Compose。为保证安全，`.env` 值永远不会被搜索或返回，敏感配置赋值行不会进入 Compose 结果，搜索词不会写入日志，也不会用于构造文件路径。
