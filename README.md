...
🆕 **NEW — Instance names and per-agent stack sorting** — Dockge agents can now have a free-form display name, editable at any time from the home page (for example `Main NAS` or `Backup server`). The technical endpoint remains visible underneath and continues to handle routing unchanged. The custom name is also used on stack pages and in the stack list. A new selector sorts stacks by status or groups them by instance, with the choice saved in the browser.
