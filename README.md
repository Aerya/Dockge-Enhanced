...
- **🔓 Restic lock auto-unlock** — A stale restic lock (exit code 11) would previously block both the backup and the `forget --prune` step entirely. Dockge Enhanced now runs `restic unlock` automatically before each operation (backup start and forget/prune), so a leftover lock from a previous crash or timeout can never block the next run.
