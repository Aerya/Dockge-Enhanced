...
**2026-07-22 — Recovery tests and application profiles** — Cold replicas can periodically read their complete encrypted Restic archive from the target to prove that the snapshot is actually recoverable. The test can also be started manually, with its date, bytes read and any error persisted. The transfer wizard now proposes PostgreSQL, MariaDB/MySQL, Redis and SQLite profiles: preparation and cleanup commands remain visible and editable before activation and only run inside the selected Compose service.
