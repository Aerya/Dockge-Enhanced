...
- **↩ Image rollback** — After each auto-update, the previous image is kept for 24 h. A countdown and a **Rollback** button appear directly in the watcher table. Old images are automatically deleted when the window expires.
- **📦 Volume backup** — The Restic backup tab now lets you include full Docker volume directories alongside the existing compose/env backups. `/app/data` (Dockge data) or `/opt/stacks` with **per-stack selection**: pick only the stacks you want, calculate individual folder sizes on demand, and toggle all/none in one click.
- **🗑️ Purge unused images** — New button in `/resources → Images` to remove all images not referenced by any active container (tagged or untagged), distinct from the existing dangling-only purge.
- **🎨 Watcher UI** — Apprise section is now collapsible (state saved per browser). Surveillance / Security / Backup tabs have been redesigned for better readability.
...
**☁️ Restic Backup** — Automatic backup of all stack `compose.yaml` and `.env` files with [Restic](https://restic.net/). **Multiple destinations in parallel** — add as many as you want (e.g. local + SFTP) and all are backed up on every run. 4 destination types: local, SFTP/NAS, S3/Backblaze B2, REST Server. SFTP supports both **SSH key** and **password** authentication (any port, `sshpass` is bundled). Configurable retention policy. **Volume backup**: optionally include `/app/data` (Dockge data) and/or `/opt/stacks` — for stacks, a **per-stack selector** lets you pick exactly which folders to include, calculate their sizes on demand, and select all/none in one click. The next scheduled backup time is displayed in the UI. Click any snapshot to expand it and see each file with two status indicators: **vs previous snapshot** (New / Modified / Unchanged) and **vs current disk** (OK / Modified / Missing). Select individual files and restore them in one click.
