...
- **📦 Volume backup** — The Restic backup tab now lets you include extra Docker volume directories alongside the existing compose/env backups: tick `/app/data` (Dockge data) and/or add any number of custom paths (e.g. `/dockers-data`), with live size display on demand.
