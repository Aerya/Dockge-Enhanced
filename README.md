...
🆕 **Homelab capacity dashboard** — A new optional **Capacity** tab in `/watcher` tracks known storage usage by stack, Docker volume, image and container logs, then highlights growth, orphan volumes, old unused images and the top consumers. It is disabled by default: manual scans are always available, scheduled scans only run after enabling the feature, and automatic scans are skipped while Synology / low-power mode is active unless explicitly allowed. Bind mount measurement is also opt-in because it can be expensive and only works for paths visible from the Dockge container.
