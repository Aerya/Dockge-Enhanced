...
- **🔓 Déverrouillage automatique du repo restic** — Un verrou restic obsolète (code de sortie 11) bloquait auparavant le backup et l'étape `forget --prune`. Dockge Enhanced exécute désormais `restic unlock` automatiquement avant chaque opération (démarrage du backup et forget/prune), de sorte qu'un verrou laissé par un crash ou timeout précédent ne peut plus bloquer la prochaine exécution.
