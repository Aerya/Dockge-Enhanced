...
- **📦 Backup des volumes** — L'onglet Backup Restic permet désormais d'inclure les répertoires de volumes Docker complets (`/app/data`, `/opt/stacks`) en plus des compose/env, avec affichage des tailles en temps réel.
...
**☁️ Backup Restic** — Sauvegarde automatique des `compose.yaml` et `.env` de chaque stack avec [Restic](https://restic.net/). **Plusieurs destinations en parallèle** — ajoutes-en autant que tu veux (ex : local + SFTP) et toutes sont sauvegardées à chaque exécution. 4 types de destination : local, SFTP/NAS (tout port, clé SSH ou mot de passe, `sshpass` intégré), S3/Backblaze B2, REST Server. **Backup des volumes** : inclus optionnellement les répertoires complets (`/app/data` pour les données Dockge, `/opt/stacks` pour toutes les stacks) — les tailles sont affichées et actualisables à la demande. Politique de rétention configurable. La date du prochain backup est affichée. Clique sur un snapshot pour le dérouler et voir chaque fichier avec deux indicateurs : **vs snapshot précédent** (Nouveau / Modifié / Inchangé) et **vs disque actuel** (OK / Modifié / Absent). Sélectionne des fichiers individuellement et restaure-les en un clic.
