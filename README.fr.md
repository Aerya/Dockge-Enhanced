...
🆕 **Navigateur et éditeur de fichiers de volumes** — Sur chaque page de stack, chaque conteneur dispose d'un bouton **Fichiers** qui ouvre un explorateur des volumes montés (binds et volumes nommés) directement dans la WebUI. On parcourt l'arborescence, on ouvre un fichier texte (JSON, conf, yaml…) dans un éditeur intégré, et on l'enregistre — sans lancer de console en parallèle. L'accès passe par le socket Docker (`docker exec`/`inspect`), donc ça marche même si le chemin hôte n'est pas monté dans le conteneur Dockge ; l'écriture se fait en place (`cat >`) pour préserver propriétaire et permissions. Restreint aux points de montage du conteneur, fichiers texte jusqu'à 512 Kio, et journalisé dans l'audit.
