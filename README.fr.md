...
🆕 **NEW — Copie et déplacement de stacks entre instances** — Depuis une stack, un assistant permet de copier ou déplacer sa configuration vers une autre instance Dockge, avec choix du nom cible. Le déploiement est facultatif pour une copie et obligatoire pour valider un déplacement. Dockge-Enhanced inventorie automatiquement les bind mounts, volumes nommés et volumes externes à partir du Compose et de Docker, puis affiche un mapping source → cible modifiable. Des règles de préfixes persistantes automatisent les chemins propres à chaque paire d'instances. Avant l'import, la cible vérifie le Compose, les ports occupés, chemins, volumes et réseaux/configs/secrets externes. L'écriture est atomique et un échec de déploiement restaure la cible ; lors d'un déplacement, la source est seulement arrêtée et ses fichiers restent disponibles pour un retour arrière. Ce premier lot transfère uniquement la configuration et ne copie pas encore les données des volumes.
