...
- **📦 Backup des volumes** — L'onglet Backup Restic permet d'inclure les répertoires de volumes Docker en plus des compose/env. `/app/data` (données Dockge) ou `/opt/stacks` avec **sélection par stack** : choisis exactement quels dossiers inclure, calcule leurs tailles à la demande, et sélectionne tout/aucun en un clic.
...
**☁️ Backup Restic** — Sauvegarde automatique des `compose.yaml` et `.env` de chaque stack avec [Restic](https://restic.net/). **Plusieurs destinations en parallèle** — ajoutes-en autant que tu veux (ex : local + SFTP) et toutes sont sauvegardées à chaque exécution. 4 types de destination : local, SFTP/NAS (tout port, clé SSH ou mot de passe, `sshpass` intégré), S3/Backblaze B2, REST Server. **Backup des volumes** : inclus optionnellement `/app/data` (données Dockge) et/ou `/opt/stacks` — pour les stacks, un **sélecteur par dossier** te permet de choisir exactement lesquels inclure, de calculer leurs tailles à la demande et de tout sélectionner/désélectionner en un clic. Politique de rétention configurable. La date du prochain backup est affichée. Clique sur un snapshot pour le dérouler et voir chaque fichier avec deux indicateurs : **vs snapshot précédent** (Nouveau / Modifié / Inchangé) et **vs disque actuel** (OK / Modifié / Absent). Sélectionne des fichiers individuellement et restaure-les en un clic.
