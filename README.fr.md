...
**2026-08-31 — Auto-mise à jour sûre de Dockge-Enhanced** — Le nouvel onglet **Mises à jour** réunit les contrôles de Dockge-Enhanced et des images. Gardez la mise à jour de Dockge-Enhanced manuelle, ou activez le mode Sidecar volontairement limité, immédiatement ou dans un créneau et les jours sélectionnés. Un backup Restic est obligatoire avant chaque mise à jour ; elle est reportée si une opération d’image, un backup ou une vérification Restic, ou un scan Trivy est en cours, et les notifications Discord et Apprise existantes signalent disponibilité, report et résultat. Le sidecar n’expose aucun port, n’accepte qu’un plan signé pour son propre conteneur Dockge-Enhanced et privilégie le projet Docker Compose existant de la stack ; si cette configuration Compose est inaccessible, il utilise un snapshot Docker qui préserve la configuration inspectée du conteneur mais ne peut pas refléter les modifications ultérieures du fichier Compose. Un healthcheck en échec restaure automatiquement le conteneur précédent. La pause globale concerne Dockge-Enhanced et toutes les images Docker gérées ; une image peut aussi être suspendue temporairement. L’option Remote Agent reste visible mais indisponible tant qu’un agent hôte dédié n’existe pas.
