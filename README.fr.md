...
**2026-07-22 — Tests de reprise et profils applicatifs** — Les répliques froides peuvent relire périodiquement l'intégralité de leur archive Restic chiffrée depuis la cible pour prouver que le snapshot est réellement restaurable. Le test est aussi lançable manuellement, avec date, volume relu et erreur persistés. L'assistant propose désormais des profils PostgreSQL, MariaDB/MySQL, Redis et SQLite : leurs commandes de préparation et de nettoyage restent visibles et modifiables avant activation, et s'exécutent uniquement dans le service Compose choisi.
...
**2026-07-22 — Transports de migration unifiés** — Le moteur de transfert utilise maintenant une interface unique `prepare/upload/restore/cleanup`. Les dépôts Restic REST fournissent le transport HTTP, les dépôts SFTP le transport SSH, et S3 ou les dépôts locaux utilisent exactement le même workflow. Tous les modes sont chiffrés, vérifiés par checksum, incrémentaux et relançables sans transporter les données volumineuses dans Socket.IO. Aucun shell SSH arbitraire n'est exposé par la WebUI.

**2026-07-17 — Filtre de stacks par instance** — La liste des stacks peut afficher toutes les instances ou une seule instance Dockge. Les compteurs Stacks, Actives, Arrêtées et Inactives se recalculent selon l'instance choisie ; le filtre est mémorisé dans le navigateur et reste indépendant du tri.
