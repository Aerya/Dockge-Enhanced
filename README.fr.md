...
🆕 **NEW — Noms d'instances et tri par agent** — Les agents Dockge peuvent recevoir un nom d'affichage libre, modifiable à tout moment depuis la page d'accueil (par exemple `NAS principal` ou `Serveur de secours`). L'endpoint technique reste visible en dessous et continue d'assurer le routage sans être modifié. Le nom personnalisé apparaît aussi sur les pages et dans la liste des stacks. Un nouveau sélecteur permet de trier les stacks par statut ou de les regrouper par instance ; le choix est mémorisé dans le navigateur.
