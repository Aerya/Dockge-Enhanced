...
**2026-08-27 — Prérequis hôte par stack au démarrage** — Les stacks gérées peuvent vérifier facultativement des points de montage hôte et services `systemd` avant tout démarrage, redémarrage ou recréation. Un prérequis « Montage hôte » doit être le point de montage exact visible sur l’hôte Docker, pas un sous-dossier : valide `/home/user/Montages/NAS/Torrent` ; invalide `/home/user/Montages/NAS/Torrent/downloads`. Le chemin désigne l’hôte Docker, pas le conteneur Dockge ; le montage existant `/var/run/docker.sock:/var/run/docker.sock` suffit et aucun bind mount de répertoire hôte supplémentaire n’est nécessaire. Le même garde protège les démarrages planifiés des stacks et services, tandis que Stop, Down et Delete restent disponibles. Une surveillance continue facultative arrête une stack active seulement après le délai de panne configuré, puis la redémarre après récupération uniquement si le watcher l’avait lui-même arrêtée ; après un redémarrage de Dockge, aucune stack déjà arrêtée n’est considérée comme lui appartenant. Les vérifications passent par des helpers Docker éphémères isolés du réseau, interrogent l’hôte, se testent depuis la page de la stack et ne changent aucun comportement tant qu’elles ne sont pas activées.
