...
🆕 **Tableau de capacité homelab** — Un nouvel onglet optionnel **Capacité** dans `/watcher` suit le stockage connu par stack, volume Docker, image et logs de containers, puis met en avant la croissance, les volumes orphelins, les images anciennes inutilisées et les plus gros consommateurs. Il est désactivé par défaut : les scans manuels restent disponibles, les scans planifiés ne tournent qu'après activation, et les scans automatiques sont ignorés en mode Synology / faible consommation sauf autorisation explicite. La mesure des bind mounts est aussi optionnelle, car elle peut coûter cher et ne fonctionne que pour les chemins visibles depuis le container Dockge.
